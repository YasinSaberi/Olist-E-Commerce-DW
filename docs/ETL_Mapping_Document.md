...
## فهرست مطالب
1. [مقدمه و دامنه سند](#۰-مقدمه-و-دامنه-سند)
2. [فلوچارت کلی پایپ‌لاین ETL](#فلوچارت-کلی-پایپلاین-etl)
3. [نگاشت منبع به استیجینگ](#۱-نگاشت-از-لایه-منبع-به-استیجینگ-source-to-staging)
4. [نگاشت ابعاد انبار داده](#۲-نگاشت-ابعاد-انبار-داده-staging-to-dimensions)
5. [نگاشت فکت‌ها](#۳-نگاشت-فکتها-و-دیتا-مارتها-staging-to-facts)
6. [کنترل کیفیت داده و مدیریت خطا](#۴-کنترل-کیفیت-داده-و-مدیریت-خطا)
7. [واژه‌نامه](#۵-واژهنامه)
...
| **`dim_review`** | `stg_reviews` | Type 0 (Fixed / Reference) | نگاشت مستقیم بدون تغییر. به‌منظور حفظ یکپارچگی ارجاعی (Referential Integrity) در `fact_logistics`، یک رکورد پیش‌فرض با شناسه‌ی `review_sk = -1` برای سفارش‌های فاقد نظر ثبت‌شده تزریق می‌شود. |
| **`dim_date`** | *تولید رویه‌ای (بدون منبع Staging)* | N/A | تولید خودکار توسط حلقه‌ی `WHILE` در T-SQL برای بازه‌ی ۲۰۱۶ تا ۲۰۱۹ (یک سال حاشیه‌ی امن برای تاریخ‌های تحویل که ممکن است از سال بعد سرریز کنند). محاسبه‌ی `is_weekend` با تابع `DATEPART(WEEKDAY, ...)`. |

> **محدودیت شناخته‌شده:** `dim_date` تعطیلات رسمی برزیل را محاسبه نمی‌کند؛ فقط آخر هفته (`is_weekend`) از طریق `DATEPART` استخراج می‌شود. اگر تحلیل تاخیر تحویل نیاز به لحاظ‌کردن تعطیلات رسمی داشته باشد، باید یک جدول مرجع تعطیلات به‌صورت جداگانه بارگذاری و به `dim_date` join شود.
...
| جدول فکت (Fact Table) | جداول لایه استیجینگ | گرین (Grain) | منطق نگاشت و تبدیل |
| :--- | :--- | :--- | :--- |
| **`fact_sales`** | `stg_orders`, `stg_order_items` | یک سطر به ازای هر `order_id` + `order_item_id` | استخراج SK از `dim_customer`، `dim_product` و `dim_date`. تبدیل `order_purchase_timestamp` به کلید صحیح `order_date_sk`. استفاده از مکانیزم `WHERE NOT EXISTS (order_id, order_item_id)` برای جلوگیری از درج رکوردهای تکراری در اجراهای مجدد. |
| **`fact_logistics`** | `stg_orders`, `stg_order_items` | یک سطر به ازای هر `order_id` (تجمیع‌شده در سطح سفارش) | محاسبه‌ی تاخیر تحویل با فرمول: `DATEDIFF(day, order_estimated_delivery_date, order_delivered_customer_date)`. جایگزینی مقادیر NULL در `seller_sk` یا `review_sk` با رکورد `-1` جهت حفظ یکپارچگی دیتامارت. مشابه `fact_sales`، از `WHERE NOT EXISTS (order_id)` برای Idempotency استفاده می‌شود. |
...
| اجرای مجدد پایپ‌لاین (Re-run) | بند `WHERE NOT EXISTS` در تمام Insertهای فکت، برای تضمین Idempotency |

## ۵. واژه‌نامه
- **SK (Surrogate Key):** کلید جایگزین عددی، مستقل از کلید طبیعی منبع، که در انبار داده تولید می‌شود.
- **SCD (Slowly Changing Dimension):** الگوی مدیریت تغییرات تاریخی در ابعاد. Type 1 = بازنویسی بدون تاریخچه، Type 2 = درج سطر جدید با بازه‌ی اعتبار، Type 3 = نگه‌داری فقط یک نسخه‌ی قبلی در ستون جداگانه.
- **Idempotent Load:** بارگذاری‌ای که اجرای مجدد آن روی داده‌ی یکسان، رکورد تکراری تولید نمی‌کند.
- **Grain:** سطح جزئیات یک ردیف در جدول فکت (مثلاً هر ردیف معادل چیست).
