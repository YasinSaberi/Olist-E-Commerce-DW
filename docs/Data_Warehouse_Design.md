...
> **ملاحظه‌ی طراحی:** چون `fact_logistics` فاقد `customer_sk` و `product_sk` است، پرسش‌هایی مانند «تاخیر تحویل به تفکیک شهر مشتری» یا «تاخیر تحویل به تفکیک دسته‌بندی محصول» مستقیماً از این فکت قابل پاسخ‌گویی نیستند و نیازمند Join با `fact_sales` از طریق `order_id` هستند. اگر این نوع تحلیل در محدوده‌ی نیازهای گزارش‌گیری است، افزودن `customer_sk` و `product_sk` (یا حداقل `customer_sk`) به `fact_logistics` در نسخه‌ی بعدی طراحی پیشنهاد می‌شود.
